...
internal class LinearRegressionTests
{
    ...
    public void Test_LinearRegression_MatchesExcel()
    {
        ...
        reg.Slope.Should().BeApproximately(0.4, 0.0001);
        reg.Offset.Should().BeApproximately(1.5429, 0.0001);
        reg.Rsquared.Should().BeApproximately(0.9074, 0.0001);
    }
    ...
    public void Test_LinearRegression_Coordinates()
    {
        ...
        LinearRegression reg = new LinearRegression(coordinates);
        reg.Slope.Should().BeApproximately(0.4, 0.0001);
        reg.Offset.Should().BeApproximately(1.5429, 0.0001);
        reg.Rsquared.Should().BeApproximately(0.9074, 0.0001);
    }
    ...
    {
        double[] ys = [2, 2, 3, 3, 3.8, 4.2, 4];
        LinearRegression reg = new LinearRegression(ys, 1, 1);
        reg.Slope.Should().BeApproximately(0.4, 0.0001);
        reg.Offset.Should().BeApproximately(1.5429, 0.0001);
        reg.Rsquared.Should().BeApproximately(0.9074, 0.0001);
    }
