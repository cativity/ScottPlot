...
public class FileFontResolver(string name, string path, bool bold, bool italic) : IFontResolver
{
    ...
    private string FontPath { get; } = File.Exists(path) ? Path.GetFullPath(path) : throw new FileNotFoundException(path);
    ...
    public SKTypeface? CreateTypeface(string fontName, bool bold, bool italic)
        => FontName == fontName && Bold == bold && Italic == italic ? SKTypeface.FromFile(FontPath) : null;

    public override string ToString() => $"File font resolver: name='{FontName}', bold={Bold}, italic={Italic}";
}
