...
public static class ColormapExtensions
{
    ...
    {
        uint[] argbs = Enumerable.Range(0, 256).Select(i => colormap.GetColor((vertical ? 255 - i : i) / 255f).ARGB).ToArray();
    ...
    ///     The starting fraction in the colormap range from which to begin extracting colors (normalized
    ///     to [0, 1]).
    /// </param>
    /// <param name="maxFraction">
    ///     The ending fraction in the colormap range at which to stop extracting colors (normalized to
    ///     [0, 1]).
    /// </param>
    public static Color[] GetColors(this IColormap colormap, int count, double minFraction = 0, double maxFraction = 1)
    {
        ...
        {
            throw new ArgumentException($"{nameof(minFraction)} must be a real number", nameof(minFraction));
        }

        if (double.IsInfinity(maxFraction) || double.IsNaN(maxFraction))
        {
            throw new ArgumentException($"{nameof(maxFraction)} must be a real number", nameof(maxFraction));
        }
        ...
        return Enumerable.Range(0, count).Select(i => colormap.GetColor((i * fractionStep) + minFraction)).ToArray();
    }
}
