...
public class UserInputProcessor
{
    ...
    public void RemoveAll<T>()
        where T : IUserActionResponse
    {
        UserActionResponses.RemoveAll(static x => x is T);
    }
    ...
    public static List<IUserActionResponse> DefaultUserResponses()
        => [
            // drag events
            new MouseDragZoomRectangle(StandardMouseButtons.Middle),
            new MouseDragPan(StandardMouseButtons.Left),
            new MouseDragZoom(StandardMouseButtons.Right),

            // click events
            new MouseWheelZoom(StandardKeys.Shift, StandardKeys.Control),
            new SingleClickAutoscale(StandardMouseButtons.Middle),
            new SingleClickContextMenu(StandardMouseButtons.Right),
            new DoubleClickBenchmark(StandardMouseButtons.Left),

            // keypress events
            new KeyboardPanAndZoom(),
            new KeyboardAutoscale(StandardKeys.A),
        ];
    ...
    /// </summary>
    private IUserActionResponse? _primaryResponse;
    ...
    {
        if (userAction is KeyDown keyDown)
        {
            KeyState.Add(keyDown.Key);
        }

        if (userAction is KeyUp keyUp)
        {
    ...
    private bool ExecuteUserInputResponses(IUserAction userAction)
    {
        ...
        lock (Plot.Sync)
        {
            ...
            {
                if (_primaryResponse is not null && _primaryResponse != response)
                {
                    continue;
                }
                ...
                if (_primaryResponse is null && info.IsPrimary)
                {
                    _primaryResponse = response;
                }

                if (_primaryResponse == response && !info.IsPrimary)
                {
                    _primaryResponse = null;
                }
            }
        }
