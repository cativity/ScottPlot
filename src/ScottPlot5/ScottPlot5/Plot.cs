...
public class Plot : IDisposable
{
    ...
    /// </summary>
    public BackgroundStyle FigureBackground { get; set; } = new BackgroundStyle { Color = Colors.White };

    /// <summary>
    ///     Style for the data area (the area within the axis frames)
    /// </summary>
    public BackgroundStyle DataBackground { get; set; } = new BackgroundStyle { Color = Colors.Transparent };
    ...
    //public PlotStyler Style { get; } = new PlotStyler();
    ...
    public Plot()
    {
        Axes = new AxisManager(this);
        Add = new PlottableAdder(this);
        Font = new FontStyler(this);
    ...
    public CoordinateRect GetCoordinateRect(Pixel pixel, float radius = 10, IXAxis? xAxis = null, IYAxis? yAxis = null)
        => GetCoordinateRect(pixel.X, pixel.Y, radius, xAxis, yAxis);
    ...
    public IAxis? GetAxis(Pixel pixel) => GetPanel(pixel, true) as IAxis;
    ...
    {
        Layout layout = RenderManager.LastRender.Layout;
        PixelRect dataRect = layout.DataRect;

        // Reverse here so the "highest" axis is returned in the case some overlap.
        IEnumerable<IPanel> panels = axesOnly ? Axes.GetPanels().Reverse().OfType<IAxis>() : Axes.GetPanels().Reverse();

        return panels.FirstOrDefault(panel => panel.GetPanelRect(dataRect, layout.PanelSizes[panel], layout.PanelOffsets[panel]).Contains(pixel));
    }
    ...
    //          + "or Render() to render onto an existing SkiaSharp surface or canvas.",
    //          true)]
    ...
    public byte[] GetImageBytes(int width, int height, ImageFormat format = ImageFormat.Bmp)
    {
        ...
        return image.GetImageBytes(format);
    }
    ...
    /// </summary>
    public IEnumerable<IPlottable> GetPlottables() => PlottableList;

    /// <summary>
    ///     Return all plottables in <see cref="PlottableList" /> of the given type.
    /// </summary>
    public IEnumerable<T> GetPlottables<T>()
        where T : IPlottable
        => PlottableList.OfType<T>();
    ...
    {
        foreach (IPlottable item in PlottableList.Where(x => x.GetType() == plotType).ToList())
        {
    ...
    public void Remove<T>()
        where T : IPlottable
    {
        PlottableList.RemoveAll(static x => x is T);
    }
    ...
    {
        Axes.AllGrids.ForEach(static x => x.IsVisible = false);
    }
    ...
    {
        Axes.AllGrids.ForEach(static x => x.IsVisible = true);
    }
    ...
    public LegendPanel ShowLegend(Edge edge)
    {
        ...
        Legend.Orientation = edge.IsVertical() ? Orientation.Vertical : Orientation.Horizontal;

        LegendPanel legendPanel = new LegendPanel(Legend) { Edge = edge, Alignment = Alignment.MiddleCenter, };
