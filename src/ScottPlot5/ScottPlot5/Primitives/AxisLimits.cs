...
public readonly struct AxisLimits : IEquatable<AxisLimits>
{
    ...
    public Coordinates Center => new Coordinates(HorizontalCenter, VerticalCenter);
    ...
    public CoordinateRect Rect => new CoordinateRect(Left, Right, Bottom, Top);

    public static AxisLimits Default { get; } = new AxisLimits(-10, 10, -10, 10);
    ...
    {
        ExpandingAxisLimits limits = new ExpandingAxisLimits(plottables);
        Left = limits.Left;
        Right = limits.Right;
    ...
    public AxisLimits InvertedVertically() => new AxisLimits(Left, Right, Top, Bottom);

    public AxisLimits InvertedHorizontally() => new AxisLimits(Right, Left, Bottom, Top);

    public AxisLimits ExpandedToInclude(AxisLimits otherLimits)
    {
        ExpandingAxisLimits expandingLimits = new ExpandingAxisLimits(this);
        expandingLimits.Expand(otherLimits);

        return expandingLimits.AxisLimits;
    }

    public static AxisLimits FromPoint(double x, double y) => new AxisLimits(x, x, y, y);

    public static AxisLimits FromPoint(Coordinates c) => new AxisLimits(c.X, c.X, c.Y, c.Y);

    public override string ToString() => $"AxisLimits: X=[{Rect.Left}, {Rect.Right}], Y=[{Rect.Bottom}, {Rect.Top}]";
    ...
    public static AxisLimits NoLimits => new AxisLimits(double.NaN, double.NaN, double.NaN, double.NaN);

    public static AxisLimits Unset => new AxisLimits(double.PositiveInfinity, double.NegativeInfinity, double.PositiveInfinity, double.NegativeInfinity);

    public static AxisLimits VerticalOnly(double yMin, double yMax) => new AxisLimits(double.NaN, double.NaN, yMin, yMax);

    public static AxisLimits HorizontalOnly(double xMin, double xMax) => new AxisLimits(xMin, xMax, double.NaN, double.NaN);

    // TODO: obsolete all Expanded() methods and replace functionality with ExpandingAxisLimits

    /// <summary>
    ///     Return a new <see cref="AxisLimits" /> expanded to include the given <paramref name="x" /> and
    ///     <paramref name="y" />.
    /// </summary>
    ...
    /// </summary>
    public AxisLimits Expanded(Coordinates coordinates) => Expanded(coordinates.X, coordinates.Y);

    /// <summary>
    ///     Return a new <see cref="AxisLimits" /> expanded to include all corners of the given <paramref name="rect" />.
    /// </summary>
    public AxisLimits Expanded(CoordinateRect rect) => Expanded(rect.TopLeft).Expanded(rect.BottomRight);
    ...
    public AxisLimits WithPan(double deltaX, double deltaY) => new AxisLimits(Left + deltaX, Right + deltaX, Bottom + deltaY, Top + deltaY);

    public AxisLimits WithZoom(double fracX, double fracY) => WithZoom(fracX, fracY, Rect.HorizontalCenter, Rect.VerticalCenter);
    ...
    public bool Equals(AxisLimits other) => Equals(Left, other.Left) && Equals(Right, other.Right) && Equals(Top, other.Top) && Equals(Bottom, other.Bottom);

    public override bool Equals(object? obj)
    {
        return obj is AxisLimits other && Equals(other);
    }

    public static bool operator ==(AxisLimits a, AxisLimits b) => a.Equals(b);
