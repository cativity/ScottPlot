...
public struct Pixel : IEquatable<Pixel>
{
    ...
    public override readonly string ToString() => $"Pixel {{ X = {X}, Y = {Y} }}";

    /// <summary>
    ///     Represents an invalid pixel location
    /// </summary>
    public static Pixel NaN => new Pixel(float.NaN, float.NaN);

    public static Pixel Zero => new Pixel(0, 0);

    /// <summary>
    ///     Convert the ScottPlot pixel to a SkiaSharp point
    /// </summary>
    public readonly SKPoint ToSKPoint() => new SKPoint(X, Y);

    public readonly bool Equals(Pixel other) => Equals(X, other.X) && Equals(Y, other.Y);
    ...
    public static Pixel operator +(Pixel a, Pixel b) => new Pixel(a.X + b.X, a.Y + b.Y);

    public static Pixel operator -(Pixel a, Pixel b) => new Pixel(a.X - b.X, a.Y - b.Y);

    public static Pixel operator *(Pixel a, float b) => new Pixel(a.X * b, a.Y * b);
    ...
    public readonly Pixel WithOffset(float dX, float dY) => new Pixel(X + dX, Y + dY);

    public readonly Pixel WithOffset(PixelOffset offset) => new Pixel(X + offset.X, Y + offset.Y);

    public readonly Pixel MovedRight(float dX) => new Pixel(X + dX, Y);

    public readonly Pixel MovedLeft(float dX) => new Pixel(X - dX, Y);

    public readonly Pixel MovedUp(float dY) => new Pixel(X, Y - dY);

    public readonly Pixel MovedDown(float dY) => new Pixel(X, Y + dY);
