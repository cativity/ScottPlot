...
public readonly struct CoordinateLine
{
    ...
    public Coordinates Start => new Coordinates(X1, Y1);

    public Coordinates End => new Coordinates(X2, Y2);
    ...
    public override string ToString() => $"CoordinateLine from ({X1}, {Y1}) to ({X2}, {Y2})";
    ...
    public CoordinateLine ExtendTo(CoordinateRect rect)
    {
        ...
        Coordinates bottom = new Coordinates(xAtBottom, rect.Bottom);

        double dTopX = rect.Top - Y1;
        double xAtTop = X1 + (dTopX * Slope);
        Coordinates top = new Coordinates(xAtTop, rect.Top);
    ...
    /// </summary>
    public double Y(double x = 0) => (Slope * x) + YIntercept;

    public CoordinateLine WithDelta(double dX, double dY) => new CoordinateLine(X1 + dX, Y1 + dY, X2 + dX, Y2 + dY);
}
