...
public readonly struct PixelLine
{
    ...
    public Pixel Pixel1 => new Pixel(X1, Y1);

    public Pixel Pixel2 => new Pixel(X2, Y2);
    ...
    public override string ToString() => $"PixelLine from ({X1}, {Y1}) to ({X2}, {Y2})";
    ...
    public PixelLine WithDelta(float dx, float dy) => new PixelLine(X1 + dx, Y1 + dy, X2 + dx, Y2 + dy);
    ...
    /// </summary>
    public float Y(float x = 0) => (Slope * x) + YIntercept;
}
