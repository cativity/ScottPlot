...
public readonly struct RenderDetails
{
    ...
    public RenderDetails(RenderPack rp, (string, TimeSpan)[] actionTimes, RenderDetails lastRender)
    {
        ...
        AxisLimitsByAxis = rp.Plot.Axes.GetAxes().ToDictionary(static x => x, static x => x.Range.ToCoordinateRange);
        Layout = rp.Layout;
    ...
    private bool AxisLimitsChangedChecker()
    {
        ...
        foreach (IAxis axis in AxisLimitsByAxis.Keys.OfType<IAxis>())
        {
            if (!PreviousAxisLimitsByAxis.TryGetValue(axis, out CoordinateRange rangeBefore)
                || AxisLimitsByAxis[axis] != rangeBefore)
            {
                return true;
            }
        }
