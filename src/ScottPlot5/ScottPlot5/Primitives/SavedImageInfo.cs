...
public class SavedImageInfo(string path, int fileSize)
{
    ...
    private RenderDetails _renderDetails;

    public override string ToString()
        => $"Saved Image [{_renderDetails.FigureRect.Width}x{_renderDetails.FigureRect.Height}] ({FileSize * .001} kB) {Path}";
    ...
    {
        string html = _imageHtmlTemplate;
        html = html.Replace("{{ REFRESH_MSEC }}", $"{refresh * 1000}");
        html = html.Replace("{{ WIDTH }}", $"{_renderDetails.FigureRect.Width}");
        html = html.Replace("{{ HEIGHT }}", $"{_renderDetails.FigureRect.Height}");
    ...
    private const string _imageHtmlTemplate = """
                                              <!doctype html>
                                              <html lang="en">
