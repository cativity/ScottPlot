...
public class Box : IHasFill, IHasLine
{
    ...
    public void Render(RenderPack rp, SKPaint paint, IAxes axes)
    {
        ...
        // body fill
        CoordinateRect bodyRect = new CoordinateRect(Position - (Width / 2), Position + (Width / 2), BoxMin, BoxMax);
        ...
        {
            CoordinateLine lineMid = new CoordinateLine(bodyRect.Left, BoxMiddle.Value, bodyRect.Right, BoxMiddle.Value);
        ...
        {
            CoordinateLine lineMax = new CoordinateLine(Position, BoxMax, Position, WhiskerMax.Value);
            PixelLine lineMaxPx = axes.GetPixelLine(lineMax);
            LineStyle.Render(rp.Canvas, lineMaxPx, paint);

            CoordinateLine lineMaxAcross = new CoordinateLine(Position - (WhiskerSize / 2), WhiskerMax.Value, Position + (WhiskerSize / 2), WhiskerMax.Value);
        ...
        {
            CoordinateLine lineMin = new CoordinateLine(Position, BoxMin, Position, WhiskerMin.Value);
            PixelLine lineMinPx = axes.GetPixelLine(lineMin);
            LineStyle.Render(rp.Canvas, lineMinPx, paint);

            CoordinateLine lineMinAcross = new CoordinateLine(Position - (WhiskerSize / 2), WhiskerMin.Value, Position + (WhiskerSize / 2), WhiskerMin.Value);
