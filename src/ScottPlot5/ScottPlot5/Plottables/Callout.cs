...
{
    public Text LabelPlottable { get; } = new Text { LabelPadding = 5 };

    public Arrow ArrowPlottable { get; } = new Arrow();
    ...
    public Coordinates CalculateClosestAttachPoint()
    {
        ...
        Pixel[] attachPoints =
        [
            new Pixel(TextPixel.X - LabelStyle.PixelPadding.Left, TextPixel.Y + (labelSize.Height / 2)), // west
            new Pixel(TextPixel.X + labelSize.Width + LabelStyle.PixelPadding.Right, TextPixel.Y + (labelSize.Height / 2)), //east
            new Pixel(TextPixel.X + (labelSize.Width / 2), TextPixel.Y - LabelStyle.PixelPadding.Top), //north
            new Pixel(TextPixel.X + (labelSize.Width / 2), TextPixel.Y + labelSize.Height + LabelStyle.PixelPadding.Bottom), // south
            new Pixel(TextPixel.X - LabelStyle.PixelPadding.Left, TextPixel.Y - LabelStyle.PixelPadding.Top), // northWest
            new Pixel(TextPixel.X + labelSize.Width + LabelStyle.PixelPadding.Right, TextPixel.Y - LabelStyle.PixelPadding.Top), // northEast
            new Pixel(TextPixel.X - LabelStyle.PixelPadding.Left, TextPixel.Y + labelSize.Height + LabelStyle.PixelPadding.Bottom), // southWest
            new Pixel(TextPixel.X + labelSize.Width + LabelStyle.PixelPadding.Right,
                      TextPixel.Y + labelSize.Height + LabelStyle.PixelPadding.Bottom) // southEast
        ];
