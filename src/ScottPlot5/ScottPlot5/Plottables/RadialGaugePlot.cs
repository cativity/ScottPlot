...
public class RadialGaugePlot : IPlottable
{
    ...
    /// <summary>
    ///     Determines whether the gauges are drawn stacked (dafault value), sequentially, or as a single gauge (ressembling a
    ///     pie plot).
    /// </summary>
    ...
    /// </summary>
    public readonly FontStyle Font = new FontStyle();
    ...
    /// </summary>
    public SKStrokeCap EndCap { get; set; } = SKStrokeCap.Round;

    /// <summary>
    ///     Style of the base of the gauge
    /// </summary>
    public SKStrokeCap StartCap { get; set; } = SKStrokeCap.Round;
    ...
    /// </summary>
    private static (double[] startAngles, double[] sweepAngles, double backStartAngle) GetGaugeAngles(double[] values,
                                                                                                      double angleStart,
                                                                                                      double angleRange,
                                                                                                      bool clockwise,
                                                                                                      RadialGaugeMode mode)
    {
        ...
        double scaleMax = values.Max(Math.Abs);

        if (mode is RadialGaugeMode.Sequential or RadialGaugeMode.SingleGauge)
        {
            scaleMax = values.Sum(Math.Abs);
            scaleMin = 0;
        }
    ...
    public IEnumerable<LegendItem> LegendItems
    {
        get
        {
            return Labels?.Select((t, i) => new LegendItem { LabelText = t, LineColor = Colors[i], LineWidth = 10, MarkerStyle = MarkerStyle.None }).ToList() ?? LegendItem.None;

            //List<LegendItem> legendItems = [];

            //for (int i = 0; i < Labels.Length; i++)
            //{
            //    LegendItem item = new LegendItem() { LabelText = Labels[i], LineColor = Colors[i], LineWidth = 10, MarkerStyle = MarkerStyle.None };
    ...
    public virtual void Render(RenderPack rp)
    {
        ...
        (double[] startAngles, double[] sweepAngles, double startingAngleBackGauges) =
            GetGaugeAngles(Levels, StartingAngle, MaximumAngle, Clockwise, GaugeMode);
        ...
        for (int i = 0; i < GaugeCount; i++)
        {
            ...
            RadialGauge gauge = new RadialGauge
            {
                ...
                Color = Colors[index],
                BackgroundColor = new Color(Colors[index].R, Colors[index].G, Colors[index].B, backgroundAlpha),
                Width = gaugeWidth,
