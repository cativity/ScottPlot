...
{
    private readonly SignalConstSource<T> _data = new SignalConstSource<T>(ys, period);
    ...
    public int MinRenderIndex { get => _data.MinRenderIndex; set => _data.MinRenderIndex = value; }

    public int MaxRenderIndex { get => _data.MaxRenderIndex; set => _data.MaxRenderIndex = value; }
    ...
    public virtual void Render(RenderPack rp)
    {
        ...
        foreach (PixelColumn col in cols)
        {
            ...
            {
                points.Add(new Pixel(col.X, col.Enter));
            }
        }
    ...
    private double PointsPerPixel() => GetVisibleXRange(Axes.DataRect).Span / Axes.DataRect.Width / _data.Period;
}
