...
public class PolarAxis : IPlottable, IManagesAxisLimits
{
    ...
    public void RegenerateSpokes(int count = 5)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            PolarAxisSpoke spoke = new PolarAxisSpoke(angle, MaximumRadius);
            Spokes.Add(spoke);
        }
    }
    ...
    public void RegenerateCircles(int count = 3)
    {
        ...
        if (count == 1)
        {
            PolarAxisCircle circle = new PolarAxisCircle(MaximumRadius);
            Circles.Add(circle);
        ...
        for (int i = 0; i < count; i++)
        {
            PolarAxisCircle circle = new PolarAxisCircle(delta * (i + 1));
            Circles.Add(circle);
        }
    }
    ...
    /// </summary>
    public static Coordinates GetCoordinates(double radius, double degrees)
    {
    ...
    ///     Return the X/Y position of a polar point
    /// </summary>
    public static Coordinates GetCoordinates(PolarCoordinates point) => GetCoordinates(point.Radius, point.Angle.Degrees);
    ...
    public AxisLimits GetAxisLimits()
    {
        ...
        if (Spokes.Count > 0)
        {
            double minSpokeLength = Spokes.Min(static x => x.Length);
            double maxSpokeLength = Spokes.Max(static x => x.Length);
        ...
        if (Circles.Count > 0)
        {
            double minCircleRadius = Circles.Min(static x => x.Radius);
            double maxCircleRadius = Circles.Max(static x => x.Radius);
            circleAbs = Math.Max(Math.Abs(minCircleRadius), Math.Abs(maxCircleRadius));
        }

        double maxAbs = Math.Max(spokeAbs, circleAbs);

        return maxAbs > 0 ? new AxisLimits(-spokeAbs, spokeAbs, -spokeAbs, spokeAbs) : AxisLimits.NoLimits;
    }

    public virtual void UpdateAxisLimits(Plot plot)
    {
        if (plot.Axes.Rules.OfType<SquareZoomOut>().Any())
        {
            return;
        }
        ...
        SquareZoomOut squareRule = new SquareZoomOut(Axes.XAxis, Axes.YAxis);
