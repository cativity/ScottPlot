...
public class Ellipse : IPlottable, IHasLine, IHasFill, IHasLegendText
{
    ...
    public FillStyle FillStyle { get; set; } = new FillStyle { Color = Colors.Transparent };
    ...
    public AxisLimits GetAxisLimits()
    {
        if (Rotation == 0)
        {
            return new AxisLimits(Center.ToRect(RadiusX, RadiusY));
        }

        // https://math.stackexchange.com/a/91304

        float rad = (float)(Rotation / 180.0 * Math.PI);
        double cos2 = Math.Pow(Math.Cos(rad), 2);
        double sin2 = Math.Pow(Math.Sin(rad), 2);

        double a2 = Math.Pow(RadiusX, 2);
        double b2 = Math.Pow(RadiusY, 2);

        double x = Math.Sqrt((a2 * cos2) + (b2 * sin2));
        double y = Math.Sqrt((a2 * sin2) + (b2 * cos2));

        return new AxisLimits(Center.ToRect(x, y));
    }
    ...
    public virtual void Render(RenderPack rp)
    {
        ...
        PixelRect rect = new PixelRect(-rx, rx, ry, -ry);
