...
public class SnapToTicksY(IYAxis yAxis) : IAxisRule
{
    ...
    public void Apply(RenderPack rp, bool beforeLayout)
    {
        ...
        bool inverted = rp.Plot.LastRender.AxisLimitsByAxis[YAxis].IsInverted;
        double oldTop = rp.Plot.LastRender.AxisLimitsByAxis[YAxis].Max;
        double oldBottom = rp.Plot.LastRender.AxisLimitsByAxis[YAxis].Min;
        CoordinateRangeMutable newLimits = YAxis.Range;
        ...
        {
            if (rule is LockedVertical lockedVerticalRule && lockedVerticalRule.YAxis == YAxis)
            {
                // the requested axis already has a vertical lock
                return;
            }

            if (rule is LockedTop lockedTopRule && lockedTopRule.YAxis == YAxis)
            {
                // the requested axis already has a top lock
                topIsLocked = true;
            }

            if (rule is LockedBottom lockedBottomRule && lockedBottomRule.YAxis == YAxis)
            {
        ...
        YAxis.RegenerateTicks(new PixelLength(rp.DataRect.Height));
        ticks = YAxis.TickGenerator?.Ticks.Where(static tick => tick.IsMajor).Select(static x => x.Position).ToList() ?? [];
