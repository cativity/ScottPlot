...
public class Histogram
{
    ...
    /// </summary>
    public int MinOutlierCount { get; private set; }

    /// <summary>
    ///     Number of values that were greater than the upper edge of the last bin.
    /// </summary>
    public int MaxOutlierCount { get; private set; }

    /// <summary>
    ///     Create a histogram which will count values supplied by <see cref="Add(double)" /> and
    ///     <see cref="AddRange(IEnumerable{double})" />
    /// </summary>
    ...
    /// <param name="addFinalBin">
    ///     if true, one more bin will be added so values equal to <paramref name="max" /> can be counted
    ///     too
    /// </param>
    /// <remarks>
    ///     If <paramref name="min" /> and <paramref name="max" /> are the same value, the <paramref name="min" /> and
    ///     <paramref name="max" />
    ///     properties will be <paramref name="min" /> - 0.5 and <paramref name="max" /> + 0.5, respectively. This is to handle
    ///     an edge
    ...
    /// <summary>
    ///     Create a histogram with bins that can count data from <paramref name="min" /> to <paramref name="max" />
    ///     (inclusive)
    /// </summary>
    public static Histogram WithFixedBinSize(double min, double max, double binSize, bool addOutliersToEdgeBins = false)
    {
        ...
        return new Histogram(min, max, binCount, addOutliersToEdgeBins, false);
    }
    ...
    public static Histogram WithFixedBinCount(double min, double max, int binCount, bool addOutliersToEdgeBins = false)
        => new Histogram(min, max, binCount, addOutliersToEdgeBins);
