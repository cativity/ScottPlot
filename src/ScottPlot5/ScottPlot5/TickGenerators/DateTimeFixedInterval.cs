                                   ...
                                   Func<DateTime, DateTime>? getIntervalStartFunc = null) : IDateTimeTickGenerator
{
    ...
    ///     then set <see cref="Interval" /> to <see cref="Hour" />, <see cref="IntervalsPerTick" /> to 6, and provide the
    ///     function:
    ///     <code>
    /// dt => new DateTime(dt.Year, dt.Month, dt.Day);
    /// </code>
    /// </example>
    public Func<DateTime, DateTime>? GetIntervalStartFunc { get; set; } = getIntervalStartFunc;
    ...
    {
        List<Tick> ticks = [];
        HashSet<DateTime> timesWithTicks = []; // Avoid having minor and major ticks at same time
        ...
        {
            string tickLabel = LabelFormatter is null ? dt.ToString(Interval.GetDateTimeFormatString()) : LabelFormatter(dt);
