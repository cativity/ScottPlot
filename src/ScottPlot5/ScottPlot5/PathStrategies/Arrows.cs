...
public static class Arrows // TODO: use an interface to let users inject custom logic
{
    public static SKPath GetPath(IEnumerable<RootedPixelVector> vectors, ArrowStyle style, float maxBladeWidth = float.PositiveInfinity)
    {
        ...
        foreach (RootedPixelVector vector in vectors)
        {
            ...
            {
                ArrowAnchor.Center => new SKPoint(vector.Point.X - (0.5f * vector.Vector.X), vector.Point.Y - (0.5f * vector.Vector.Y)),
                ArrowAnchor.Tail => vector.Point.ToSKPoint(),
                ArrowAnchor.Tip => new SKPoint(vector.Point.X - vector.Vector.X, vector.Point.Y - vector.Vector.Y),
            ...
            SKPoint head = path.LastPoint;

            SKPoint junction = head + new SKPoint(-bladeLengthFactor * vector.Vector.X, -bladeLengthFactor * vector.Vector.Y);

            SKPoint perp = new SKPoint(-vector.Vector.Y, vector.Vector.X);
            SKPoint bladeTip1 = junction + new SKPoint(Math.Min(bladeWidthFactor * perp.X, maxBladeWidth), Math.Min(bladeWidthFactor * perp.Y, maxBladeWidth));
            SKPoint bladeTip2 = junction - new SKPoint(Math.Min(bladeWidthFactor * perp.X, maxBladeWidth), Math.Min(bladeWidthFactor * perp.Y, maxBladeWidth));

            SKPoint arrowHeadBegin = junction + new SKPoint(cutInFactor * vector.Vector.X, cutInFactor * vector.Vector.Y);
