...
public class Population : ICategory
{
    ...
    public class PopulationQuickstart : RecipeBase
    {
        ...
        public override void Execute()
        {
            for (int i = 0; i < 5; i++)
            {
                double[] values = Generate.RandomNormal(10, 3 + i);
                MyPlot.Add.Population(values, i);
            }
            ...
            string[] tickLabels = Enumerable.Range(1, 5).Select(static x => $"Group {x}").ToArray();
    ...
    public class PopulationBox : RecipeBase
    {
        ...
        public override void Execute()
        {
            ...
            {
                double[] values = Generate.RandomNormal(10, 3 + i);
                PopulationSymbol pop = MyPlot.Add.Population(values, i);
    ...
    public class PopulationBoxValues : RecipeBase
    {
        ...
        public override string Description
            => "The values displayed by the box midline, body, and whisker can be "
    ...
    public class PopulationArrangement : RecipeBase
    {
        ...
        public override string Description
            => "The user may customize where data is drawn relative to the bar or box. "
    ...
    public class PopulationGroups : RecipeBase
    {
        ...
        public override string Description => "Groups of populations can be achieved by customizing position, color, axis labels, and legend items.";
        ...
        public override void Execute()
        {
            ...
            for (int groupIndex = 0; groupIndex < groupNames.Length; groupIndex++)
            {
                ...
                {
                    double[] values = Generate.RandomNormal(10, 2 + (groupIndex * 2));
                    double x = (groupIndex * (categoryNames.Length + 1)) + categoryIndex;
                    PopulationSymbol pop = MyPlot.Add.Population(values, x);
            ...
            double[] tickPositions = Enumerable.Range(0, groupNames.Length).Select(x => (x * tickDelta) + (tickDelta / 2) - 1).ToArray();
            ...
            {
                LegendItem item = new LegendItem { FillColor = categoryColors[i], LabelText = categoryNames[i] };
