...
public class Finance : ICategory
{
    ...
    public class Candlestick : RecipeBase
    {
        ...
        public override string Description
            => "Candlestick charts use symbols to display price data. "
        ...
        {
            List<OHLC> prices = Generate.RandomOHLCs(30);
    ...
    public class OhlcChart : RecipeBase
    {
        ...
        public override string Description => "OHLC charts use symbols to display price data (open, high, low, and close) for specific time ranges.";
    ...
    public class FinanceChartBins : RecipeBase
    {
        ...
        public override string Description => "Finance charts can display price range information over arbitrary time scales.";

        [Test]
        public override void Execute()
        {
            DateTime timeOpen = new DateTime(1985, 09, 24, 9, 30, 0); // 9:30 AM
            DateTime timeClose = new DateTime(1985, 09, 24, 16, 0, 0); // 4:00 PM
    ...
    public class FinanceRightAxis : RecipeBase
    {
        ...
        public override string Description => "Finance charts can be created which display price information on the right axis.";
        ...
        public override void Execute()
        {
            ...
            List<OHLC> prices = Generate.RandomOHLCs(30);
            CandlestickPlot candles = MyPlot.Add.Candlestick(prices);
    ...
    public class FinanceSma : RecipeBase
    {
        ...
        public override string Description => "Tools exist for creating simple moving average (SMA) curves and displaying them next to finanial data.";

        [Test]
        public override void Execute()
        {
            // generate and plot time series price data
            List<OHLC> prices = Generate.RandomOHLCs(75);
            MyPlot.Add.Candlestick(prices);
            MyPlot.Axes.DateTimeTicksBottom();
    ...
    public class FinanceBollinger : RecipeBase
    {
        ...
        {
            // generate and plot time series price data
            List<OHLC> prices = Generate.RandomOHLCs(100);
            MyPlot.Add.Candlestick(prices);
            MyPlot.Axes.DateTimeTicksBottom();
            ...
            ScottPlot.Plottables.Scatter sp1 = MyPlot.Add.Scatter(bb.Dates, bb.Means);
            sp1.MarkerSize = 0;
            sp1.Color = Colors.Navy;

            // display upper bands (positive variance) as a dashed line
            ScottPlot.Plottables.Scatter sp2 = MyPlot.Add.Scatter(bb.Dates, bb.UpperValues);
            sp2.MarkerSize = 0;
            sp2.Color = Colors.Navy;
            sp2.LinePattern = LinePattern.Dotted;

            // display lower bands (positive variance) as a dashed line
            ScottPlot.Plottables.Scatter sp3 = MyPlot.Add.Scatter(bb.Dates, bb.LowerValues);
            sp3.MarkerSize = 0;
    ...
    public class FinancialPlotWithoutGaps : RecipeBase
    {
        ...
        public override string Description
            => "When the DateTimes stored in OHLC objects "
        ...
        public override void Execute()
        {
            ...
            List<OHLC> prices = Generate.RandomOHLCs(31);
            CandlestickPlot candlePlot = MyPlot.Add.Candlestick(prices);
            ...
            DateTime[] tickDates = prices.Where((_, i) => i % tickDelta == 0).Select(x => x.DateTime).ToArray();
            ...
            MyPlot.Axes.Bottom.TickGenerator = new NumericManual(tickPositions, tickLabels);
        }
    }
    ...
    public class FinancialPlotWithoutGapsOhlc : RecipeBase
    {
        ...
        public override void Execute()
        {
            // create a OHLC plot
            List<OHLC> prices = Generate.RandomOHLCs(31);
            OHLCPlot ohlcPlot = MyPlot.Add.OHLC(prices);
